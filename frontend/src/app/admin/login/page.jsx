...
const AdminLogin = () => {
    ...
    // 🎹 GLOBAL KEY LISTENER
    /* eslint-disable react-hooks/immutability -- resetSystem/triggerChaos are only invoked
       from this listener on a later keydown event, by which point the render below has
       already initialized both consts; no TDZ hit occurs at runtime. */
    useEffect(() => {
    ...
    return (
        <div className={`relative min-h-screen flex items-center justify-center overflow-hidden transition-colors duration-500 ${getContainerClass()}`}>
            ...
            {/* 2. Confetti Layer */}
            {/* eslint-disable react-hooks/purity -- decorative randomized confetti positions, intentionally re-randomized per render */}
            {activeMode === 'party' && (
