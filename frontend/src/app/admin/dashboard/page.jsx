  ...
  const fetchAllData = useCallback(async (pageNum = 1, searchQuery = "") => {
      ...
      try {
          ...
          const blogRes = await axios.get(
            `${API_URL}/api/blog/all-blogs?page=${pageNum}&limit=9&search=${searchQuery}`,
    ...
    const handleRequestOtp = async () => {
        ...
        try {
            ...
            const res = await axios.post(
                `${API_URL}/api/user/send-current-user-otp`,
                {},
                { headers: { Authorization: `Bearer ${token}` } }
            );
            if (res.data.success) {
                    ...
                    <div className="pt-2 border-t border-white/5 space-y-3">
                        ...
                        <div className="flex gap-2 w-full">
                            <input
                                ...
                                disabled={!otpSent}
                                onChange={handleChange}
                                className="flex-1 min-w-0 bg-zinc-900/50 border border-white/10 rounded-lg px-4 py-3 text-sm text-white focus:ring-1 focus:ring-emerald-500 outline-none disabled:opacity-50 font-mono tracking-widest text-center"
                            />
                            ...
                            {!otpSent ? (
                                <button
                                    ...
                                    className="shrink-0 px-4 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold rounded-lg transition-all shadow-lg shadow-indigo-500/20 whitespace-nowrap"
                                >
