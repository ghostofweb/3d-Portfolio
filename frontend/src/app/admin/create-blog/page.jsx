...
const CustomToolbar = () => (
  <div id="toolbar" className="flex flex-wrap gap-2 items-center">
    <span className="ql-formats">
      <select className="ql-header" defaultValue="" onChange={e => e.persist && e.persist()}>
        <option value="1" />
        <option value="2" />
        <option value="" />
      </select>
...
const CreateBlog = () => {
  ...
  const navigate = (path) => (path === -1 ? router.back() : router.push(path));
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/set-state-in-effect -- derives slug from title
      ...
      <main className="pt-40 md:pt-28 pb-32 max-w-4xl mx-auto px-4 md:px-6">
        <div className={viewMode === 'edit' ? 'block' : 'hidden'}>
            ...
            <div className="prose prose-invert max-w-none pb-20">
                <ReactQuill ref={quillRef} theme="snow" value={content} onChange={setContent} modules={modules} formats={formats} placeholder="Tell your story..." scrollingContainer="html"/>
            </div>
        </div>
